...
class ResBlock(TimestepBlock):
    ...
    def forward(self, x, emb):
        ...
        """
        if self.updown:
            # Up/Downsample before last conv layer
            in_rest, in_conv = self.in_layers[:-1], self.in_layers[-1]
            h = in_rest(x)
            h = self.h_upd(h)
            h = in_conv(h)

            # Up/Downsample before skip connection
            x = self.x_upd(x)
        else:
            h = self.in_layers(x)
        # time embedding
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        assert dims == 2, 'Only 2D images are currently supported'
        ...
        for level, mult in enumerate(channel_mult):
            ...
            # Use Downsample except for the last block
        ...
        for level, mult in list(enumerate(channel_mult))[::-1]:
            for i in range(num_res_blocks + 1):
                ...
                # Use Upsample except for the last block
...
if __name__ == "__main__":
    ...
    x = torch.randn(4, 3, 128, 128).to(device)  # (4, 3, 256, 256) O.O.M Error
    time_in_batch = torch.randint(0, 1000, (4,)).to(device)
    y = unet(x, time_in_batch)
    print(y.shape)  # [4, 3, 128, 128]
