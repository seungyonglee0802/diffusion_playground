...
class Model(nn.Module):
    ...
    def forward(self, x, time_step, guide=None, emb=None):
        if self.in_training:
            ...
            time_step = (
                torch.Tensor([time_step for _ in range(x.size(0))])
...
class DiffusionProcess:  # ONLY for the Inference
    ...
    def _one_diffusion_step(self, x, guide=None):
        ...
        for idx in reversed(range(len(self.alpha_bars))):
            ...
            sqrt_tilde_beta = torch.sqrt(
                (1 - self.alpha_prev_bars[idx])
                / (1 - self.alpha_bars[idx])
                * self.betas[idx]
            )
            ...
            mu_theta_xt = torch.sqrt(1 / self.alphas[idx]) * (
                x
                - self.betas[idx]
                / torch.sqrt(1 - self.alpha_bars[idx])
                * predict_epsilon
            )
    ...
    def sampling(self, sampling_batch, only_final=False):
        ...
        """
        sample = torch.randn([sampling_batch, *self.shape]).to(device=self.device)
        sampling_list = []
    ...
    for row in range(row_number):
        for col in range(col_number):
            ...
            show_sample[
                ...
                (_initial - _initial.min()) / (_initial.max() - _initial.min()) * 255
            )
            ...
            show_sample[
                ...
                (_result - _result.min()) / (_result.max() - _result.min()) * 255
            )
