...
class DDPM(nn.Module):
    ...
    def __init__(self,
        ...
        self.model = instantiate(
            OmegaConf.load(unet_config_path),
    ...
    def p_mean_variance(self, x, t, guide=None, context=None, emb=None, clip_denoised: bool = True):
        ...
        """
        x = torch.cat([x, guide], dim=1) if guide is not None else x
...
def train(ddpm_process, train_loader, max_epochs=100):
    start = time.time()
    ...
    for epoch in range(max_epochs):
        for idx, (x, guide, label) in enumerate(train_loader):
            ...
            loss = ddpm_process(x, guide=guide, emb=None)
            opt.zero_grad()
            loss.backward()
            opt.step()
...
def inference(ddpm_process, sampling_batch, return_intermediates=False, val_dataset=None):
    ...
    if val_dataset:
        ...
        _, guide, label = next(iter(val_loader)).to(ddpm_process.device)
    ...
    return guide, samples if not return_intermediates else guide, samples, denoise_row
...
if __name__ == "__main__":
    ...
    ddpm_process = DDPM(
        unet_config_path="config/model/base.yaml",
        image_size=16,
        channels=1,
        concat_channels=1,
    )
    ...
    trn_dataset = instantiate(
        OmegaConf.load("config/data/guide_mnist.yaml"),
        train=True,
    )
    ...
    # inference
    guide, result = inference(ddpm_process, 16, val_dataset=val_dataset)
